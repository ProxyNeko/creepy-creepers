package dev.nertzhul.creepycreepers.fabric.util.registry;
...
public class FabricRegistryProvider<T> implements RegistryProvider<T> {
    ...
    public FabricRegistryProvider(String pModId, Registry<T> pRegistry) {
        this.modId = pModId;
        this.registry = pRegistry;
    }
    
    public FabricRegistryProvider(String pModId, ResourceKey<? extends Registry<T>> pResourceKey) {
        this.modId = pModId;
        
        final var reg = BuiltInRegistries.REGISTRY.get(pResourceKey.location());
        if (reg == null) {
    ...
    public <I extends T> RegistryEntry<I> register(String pName, Supplier<? extends I> pSupplier) {
        ...
        final RegistryEntry<I> entry = new RegistryEntry<>() {
            final ResourceKey<I> resourceKey = ResourceKey.create((ResourceKey<? extends Registry<I>>) FabricRegistryProvider.this.registry.key(), resource);
            ...
            public Holder<I> holder() {
                return (Holder<I>) FabricRegistryProvider.this.registry.getHolder((ResourceKey<T>) this.resourceKey).orElseThrow();
            }
        };
